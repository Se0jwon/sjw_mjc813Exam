...
class Score {
// 학생 1명의 국어,영어,수학,사회,과학 5개 점수를 저장하는 클래스
    kor = 0;
    eng = 0;
    mat = 0;
    soc = 0;
    ...
    make() {
        ...
        // 41 ~ 100 점 까지 랜덤한 점수를 리턴한다
    }
    toString() {
        // 한명의 국어,영어,수학,사회,과학 점수를 문자열로 리턴한다.
        let result = `국어:${this.kor.toString().padStart(2, "0")}`;
        result += `, 영어:${this.eng.toString().padStart(2, "0")}`;
        result += `, 수학:${this.mat.toString().padStart(2, "0")}`;
        result += `, 사회:${this.sci.toString().padStart(2, "0")}`;
        result += `, 과학:${this.soc.toString().padStart(2, "0")}`;
        return result;
    }
}

class OneClass {
// 학생 Score 30명을 저장하는 클래스
    stu30 = [];
    constructor() {
        for( let i = 0; i < 30; i++ ) {
            this.stu30.push(new Score());
        }
    }
    toString(n) {
        // 학생 30명의 5개 과목 점수를 문자열로 리턴하는 메소드
        let result = `${n}반 : <br/>`;
        for( let i = 0; i < 30; i++ ) {
            result += `${this.stu30[i].toString()}<br/>`;
        }
        return result;
    }
    toAvgString(n) {
        // 학생 30명의 5개 과목 평균 점수를 문자열로 리턴하는 메소드
        let sum = new Score();
        sum.kor = 0;
        sum.eng = 0;
        sum.mat = 0;
        ...
        sum.sci = 0;
        for( let i = 0; i < 30; i++ ) {
            sum.kor += this.stu30[i].kor;
            sum.eng += this.stu30[i].eng;
            sum.mat += this.stu30[i].mat;
            sum.soc += this.stu30[i].soc;
            sum.sci += this.stu30[i].sci;
        }
        sum.kor = Math.round(sum.kor / 30);
        sum.eng = Math.round(sum.eng / 30);
        sum.mat = Math.round(sum.mat / 30);
        sum.soc = Math.round(sum.soc / 30);
        sum.sci = Math.round(sum.sci / 30);
        let result = `${n}반 : 국어:${sum.kor.toString().padStart(2, "0")}`;
        result += `, 영어:${sum.eng.toString().padStart(2, "0")}`;
        result += `, 수학:${sum.mat.toString().padStart(2, "0")}`;
        result += `, 사회:${sum.sci.toString().padStart(2, "0")}`;
        result += `, 과학:${sum.soc.toString().padStart(2, "0")}<br/>`;
        return result;
    }
}
...
class Exam91 {
    ...
    constructor() {
        for( let i = 0; i < 7; i++ ) {
            this.sevenClass.push( new OneClass() );
        }
    }
    toString() {
        // 7개 반의 정보를 문자열로 만들어서 리턴한다.
        let result = "===============================<br/>";
        for( let i = 0; i < 7; i++ ) {
            result += this.sevenClass[i].toString(i+1);
            result += "===============================<br/>";
        }
        return result;
    }
    toAvgString() {
        // 7개 반의 평균 점수 정보를 문자열로 만들어서 리턴한다.
        let result = "-----------------------------------------<br/>";
        for( let i = 0; i < 7; i++ ) {
            result += this.sevenClass[i].toAvgString(i+1);
            result += "-----------------------------------------<br/>";
        }
        return result;
    }
...
function solveExam91() {
    ...
    let result = exam91.toString();
    result += exam91.toAvgString();
    document.getElementById("exam91result").innerHTML = result;
}
...
class Exam96 {
    ...
    arr2Result = [];
    constructor(input) {
        let arr1Dm = input.split("\n");
        for( let i = 0; i < arr1Dm.length; i++ ) {
            let arr = arr1Dm[i].split(" ");
            arr = arr.map((x) => Number(x));
            this.arr2Question.push(arr);
            this.arr2Result.push(Array.from(arr))
        }
    }
    solve() {
        let findResult = false;
        let nSquare = this.arr2Question.length;
        while( findResult == false && nSquare >= 2 ) {
            for( let row = 0; findResult == false && row <= this.arr2Question.length - nSquare; row++ ) {
                for( let col = 0; findResult == false && col <= this.arr2Question.length - nSquare; col++ ) {
                    if ( this.checkArrayMax(row, col, nSquare) ) {
                        // 2차원 정방형 배열에서 [row][col] 부터 nSquare 공간까지 모두 0인것을 찾은 경우에
                        this.printResultArray(row, col, nSquare);
                        findResult = true;  // for, for, while 문을 모두 빠져나가기 위한 변수를 true 설정
                        break;
                    };
                }
            }
        ...
        return this.printResult(nSquare);
    }
    checkArrayMax(nRow, nCol, nSquare) {
    // 2차원 정방형 배열의 값이 모두 0 이면 true 를 리턴한다.
        for( let i = nRow; i < nSquare; i++ ) {
            for( let j = nCol; j < nSquare; j++ ) {
                if ( this.arr2Question[i][j] != 0 ) {
                    // 배열 원소의 값이 하나라도 0 이 아니면 false 를 리턴한다.
                    return false;
                }
            }
        }
    ...
    printResultArray(nRow, nCol, nSquare) {
        // 2차원 정방형 배열의 [nRow][nCol] 부터 +nSquare 공간까지 '#' 를 표시
        let nRowMax = nRow + nSquare;
        let nColMax = nCol + nSquare;
        for( let i = nRow; i < nRowMax; i++ ) {
            for( let j = nCol; j < nColMax; j++ ) {
                this.arr2Result[i][j] = "#";
            }
        }
    }
    printResult(nSquare) {
        let result = `${nSquare} * ${nSquare}<br/>`;
        for( let i = 0; i < this.arr2Result.length; i++ ) {
            for( let j = 0; j < this.arr2Result.length; j++ ) {
                result += `${this.arr2Result[i][j]} `;
            }
            result += "<br/>";
        }
        return result;
...
function solveExam96() {
    ...
    let exam96 = new Exam96(texts);
    let result = exam96.solve();
    document.getElementById("exam96result").innerHTML = result;
    recursiveTest();
}

function recursiveAddFunc(num) {
    // 1부터 n 까지의 합을 리턴하는 재귀함수
    // n, n-1, ... 1 => n + raF(n-1) + raF(n-2) + ... + 1
    if (num <= 1) {
        // 재귀함수가 어떤 경계 값에서 다시 자기를 호출하면 안된다.
        console.log(`recursiveAddFunc(1)`);
        return 1;
    }
    console.log(`recursiveAddFunc(${num})`);
    // 재귀함수는 경계값 안에서는 자기함수를 다시 호출한다.
    return num + recursiveAddFunc(num-1);
}

function recursiveTest() {
    // 재귀함수 만든 테스트
    let result = recursiveAddFunc(10);
    console.log(`recursiveAddFunc(10) : ${result}`);
    result = recursiveAddFunc2(1, 20);
    console.log(`recursiveAddFunc2(1, 20) : ${result}`);
}

function recursiveAddFunc2(num, max) {
    // 1, 2, ... n => 1 + raF(2) + raF(3) + ... + raF(n)
    if (num < max) {
        // 재귀함수가 어떤 경계 값 안에서는 자기함수를 다시 호출한다.
        console.log(`recursiveAddFunc2(${num}, ${max})`);
        return num + recursiveAddFunc2(num + 1, max);
    }
    // 재귀함수는 경계를 벗어나면 리터럴 값을 리턴한다.
    console.log(`recursiveAddFunc2(${num}, ${max})`);
    return num;
}